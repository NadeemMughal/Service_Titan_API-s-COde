...
def add_appointment():
    ...
    # Try a different approach with headers - several variations
    all_headers_variations = [
        # Variation 1: Standard approach
        {
            "Authorization": f"Bearer {access_token}",
            "ST-App-Key": client_id,
            "Content-Type": "application/json"
        },
        # Variation 2: Try without ST-App-Key
        {
            "Authorization": f"Bearer {access_token}",
            "Content-Type": "application/json"
        },
        # Variation 3: Try client_id in a different header
        {
            "Authorization": f"Bearer {access_token}",
            "apikey": client_id,
            "Content-Type": "application/json"
        },
        # Variation 4: Try with app-key (lowercase)
        {
            "Authorization": f"Bearer {access_token}",
            "app-key": client_id,
            "Content-Type": "application/json"
    ...
    # Request body according to ServiceTitan API documentation
    appointment_data = {
        "jobId": job_id,
    ...
    # Try each headers variation
    for i, headers in enumerate(all_headers_variations):
        print(f"\nTrying headers variation {i + 1}:")
        ...
        try:
            ...
            if response.status_code == 200:
                print(f"Success with headers variation {i + 1}")
                return response.json()

        except requests.exceptions.RequestException as e:
            print(f"Network error with headers variation {i + 1}:", str(e))

    # If we get here, all variations failed
    raise HTTPException(status_code=500, detail="All authentication attempts failed")
